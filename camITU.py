...
def verificar_nota_existente(nota_fiscal):
    ...
    if conn:
        ...
        try:
            cursor.execute(
                """
                SELECT COUNT(*) FROM notafiscaiscanhotoitu
                WHERE NumeroNota = %s
                """,
                (nota_fiscal,)
            )
...
def salvar_imagem_no_banco(imagem, nota_fiscal):
    ...
    if conn:
        ...
        try:
            ...
            cursor.execute(
                """
                INSERT INTO notafiscaiscanhotoitu (NumeroNota, DataBipe, CaminhoImagem, Imagem)
                VALUES (%s, %s, %s, %s)
                """,
    ...
    if conn:  # Verifica se a conexão foi bem-sucedida
        try:
            with conn.cursor() as cursor:
                ...
                cursor.execute("SELECT COUNT(*) FROM notafiscaiscanhotoitu")
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f"🏭 Itumbiara<br>Qtd. Canhotos:<br>🔗{quantidade_canhotos}</h3>", unsafe_allow_html=True)
